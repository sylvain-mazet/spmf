package ca.pfv.spmf.algorithms.frequentpatterns.apriori_rare;
 ...
 *   
 * and the original Apriori artice:<br/><br/>
 * 
 *   Agrawal R, Srikant R. "Fast Algorithms for Mining Association Rules", VLDB. Sep 12-15 1994, Chile, 487-99,
 * <br/><br/>
 * 
 * The AprioriRare algorithm finds all minimal rare itemsets.<br/><br/>
 * 
	...
	public ItemsetsArrayIntegerWithCount runAlgorithm(double minsup, String input, String output) throws IOException {
		...
		for(Entry<Integer, Integer> entry : mapItemCount.entrySet()){
			if(entry.getValue() >= minsupRelative){
				...
				// MODIFIED FOR MINIMUM RARE ITEMSET MINING
	 ...
	 loopCand:	for(ItemsetArrayImplWithCount candidate : candidatesK){
			...
			for (ItemsetArrayImplWithCount candidate : candidatesK) {
				...
				if (candidate.getAbsoluteSupport() >= minsupRelative) {
					...
					// MODIFIED FOR MINIMUM RARE ITEMSET MINING
					// the itemset is infrequent so save it into results
