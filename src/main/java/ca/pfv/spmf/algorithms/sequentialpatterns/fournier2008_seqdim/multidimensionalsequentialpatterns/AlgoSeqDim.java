...
public class AlgoSeqDim {
	...
	/** if true, only closed patterns are found */
	...
	/** if true the sequence identifiers of each patterns will be shown in the output file */
	...
	public MDSequences runAlgorithm(MDSequenceDatabase database,
		...
		// (2) For each frequent sequential pattern found, �
	...
	private void writeToFile(MDSequence mdsequence) throws IOException {
		...
		if(outputSequenceIdentifiers ){
			buffer.append(" #SID: ");
			for(Integer id : mdsequence.getSequence().getSequencesID()){
				buffer.append(id);
				buffer.append(' ');
			}
		}
	...
	public void printStatistics(int databaseSize) {
		StringBuilder r = new StringBuilder(140);
